...
void populateMap(world_t* world, struct Point h){

    arrE_t h1;
    initArrE(&h1, h);
    char gameBoard[21][80] = world->worldMaps[h1.row][h1.col]->screen;

    // //Firstly, we need to check the n, e, s, w points to see if 
    // //we need to match any of the gate coordinates or not
    // struct Point n;
    ...
    // if (world->worldMaps[n1.row][n1.col] != NULL){

    //     nGate = world->worldMaps[n1.row][n1.col]->nG;

    // }

    // if (world->worldMaps[s1.row][s1.col] != NULL){

    //     sGate = world->worldMaps[s1.row][s1.col]->sG;

    // }

    // if (world->worldMaps[e1.row][e1.col] != NULL){

    //     eGate = world->worldMaps[e1.row][e1.col]->eG;

    // }

    // if (world->worldMaps[w1.row][w1.col] != NULL){

    //     wGate = world->worldMaps[w1.row][w1.col]->wG;
...
void generatePaths(char gameBoard[21][80], world_t* world, struct Point p){
    ...
    if (world->worldMaps[e1.row][e1.col] != NULL){
        ...
        y1 = world->worldMaps[e1.row][e1.col]->eG;
    ...
    if (world->worldMaps[w1.row][w1.col] != NULL){
        ...
        y2 = world->worldMaps[w1.row][w1.col]->wG;
    ...
    if (world->worldMaps[n1.row][n1.col] != NULL){
        ...
        a1 = world->worldMaps[n1.row][n1.col]->nG;

    }

    if (world->worldMaps[s1.row][s1.col] != NULL){

        a2 = world->worldMaps[s1.row][s1.col]->sG;
